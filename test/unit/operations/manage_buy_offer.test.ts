...
describe("Operation.manageBuyOffer()", () => {
  ...
  it("creates a manageBuyOfferOp (string price)", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "3.1234560",
    ...
    expect(obj.type).toBe("manageBuyOffer");
    expect(obj.selling.equals(selling)).toBe(true);
    expect(obj.buying.equals(buying)).toBe(true);
    expect(operation.body().value().buyAmount().toString()).toBe("31234560");
    expect(obj.buyAmount).toBe("3.1234560");
    expect(obj.price).toBe("8.141592");
    expect(obj.offerId).toBe("1");
  });

  it("creates a manageBuyOfferOp (price fraction)", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "3.123456",
  ...
  it("creates a manageBuyOfferOp (number price)", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "3.123456",
  ...
  it("creates a manageBuyOfferOp (BigNumber price)", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "3.123456",
  ...
  it("creates a manageBuyOfferOp with no offerId (defaults to 0)", () => {
    const opts = {
      selling,
      buying,
    ...
    expect(obj.selling.equals(selling)).toBe(true);
    expect(obj.buying.equals(buying)).toBe(true);
    expect(operation.body().value().buyAmount().toString()).toBe("10000000000");
    expect(obj.buyAmount).toBe("1000.0000000");
    expect(obj.price).toBe("3.141592");
    expect(obj.offerId).toBe("0");
  });

  it("cancels offer (buyAmount 0 with offerId)", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "0.0000000",
    ...
    expect(obj.selling.equals(selling)).toBe(true);
    expect(obj.buying.equals(buying)).toBe(true);
    expect(operation.body().value().buyAmount().toString()).toBe("0");
    expect(obj.buyAmount).toBe("0.0000000");
    expect(obj.price).toBe("3.141592");
    expect(obj.offerId).toBe("1");
  });

  it("creates a manageBuyOfferOp with numeric offerId", () => {
    const opts = {
      selling,
      buying,
      buyAmount: "5.0000000",
  ...
  it("creates a manageBuyOfferOp with source account", () => {
  ...
  describe("fails to create manageBuyOffer operation", () => {
    it("throws with an invalid buyAmount", () => {
      expect(() =>
        Operation.manageBuyOffer({
          buyAmount: 20 as unknown as string,
          price: "10",
          selling,
          buying,
        }),
